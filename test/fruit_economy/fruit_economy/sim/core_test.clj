...
(comment
  ...
  (let [width 1 height 1
    ...
    #_(ffirst
        (q '[:find ?value
             :where [?e ::land/civ-name->civ ?value]]
          db)))
...
(comment
  (let [width 1 height 1
        ...
        new-civ (civ/make-civ curr-civ-id civ-name symbol civ-loc name biome parent-civ [new-peep])
        world-db {}]
    #_(keys land-data)
    (-> (d/empty-db {} #_{:decisions {:db/cardinality :db.cardinality/many}})
      (d/db-with [land-data
                  new-peep
                  new-civ]))
    #_(-> world-db
        (db/query [[:insert :land 1 land-data]
                   [:insert :civs 2 new-civ]]))))
