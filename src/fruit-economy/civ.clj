...
(def qualities [:weak :strong :hard :soft :beer :wine :flavourful :plain :spicy :salty :sweet :dry :vegetable :meat :bread :dairy :fish :fizzy :fatty :alcohol :spirit :grain :carbohydrate :protein :bean :red-meat :white-meat :crispy :wet :hot :cold :floral :fruit :herby :rich :nutty])
  ...
  (let [[x y] origin]
    (merge
      {::id id
       ...
       ::dead false
       ;; civ priorities
       ::priorities (into [] (comp (map (fn [priority] (repeat (roll 8) priority))) cat) [:expand-territory :tech-development :population-growth :territorial-stabilisation :settlement-construction])
...
(defn peep-on-tick [{id :db/id :keys [area decisions planned-decision] :as peep} world-db]
  ...
  (let [[x y] area dir-x (rand-nth [-1 0 1]) dir-y (rand-nth [-1 0 1])
    ...
    (println :decisions (seq decisions) :planned-decision planned-decision :data data)
    data))
