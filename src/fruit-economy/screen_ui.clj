...
(defn gen-prices [supply demand price-velocity]
  (let [more-demand? (> demand supply)
        more-supply? (< demand supply)]
    (cond
      (and more-demand? (> price-velocity 0))
      ;; TODO: use whole numbers instead of
      (max #_(* price-velocity 1.2) 0.01)

      more-demand?
      (max #_(* price-velocity -0.5) 0.01)

      (and more-supply? (< price-velocity 0))
      (min #_(* price-velocity 1.2) -0.01)

      more-supply?
      (min #_(* price-velocity -0.5) -0.01)
...
(comment
  (do
    (load-file "src/fruit_economy/core.clj")
    (swap! fruit-economy.core/*menu assoc :screen fruit-economy.core/game-screen)))
