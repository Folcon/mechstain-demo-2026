  ...
  ([name width height {:keys [temp-mod elev-mod temp-noise elev-noise base-biome] :or {temp-mod 0.1 elev-mod 0 base-biome :ocean}}]
   (let [temp-noise (or temp-noise (make-temp-noise-map width height))
     ...
     {:db/ident :land
      ...
      ::raw-resource (vec (repeat height (vec (repeat width {:food 0 :max-food 0 :rock 0 :max-rock 0}))))
      ::area->units {}
      :land/units []
      ::area->resources {}
      :land/resources []
      ...
      (reduce
        (fn [{::keys [temp elev] :as land} x]
          (let [local-temp (get-in temp [y x])
                ...
                resource-fn (fn [x] (if (>= local-elev sea-level) (int (* (/ (+ x 1) 2) 5)) 0))
                food (resource-fn local-temp)
                rock (resource-fn local-elev)]
            (->
              (cond
                ...
                :else
                land)
              (assoc-in [::raw-resource y x] {:food food :max-food food :rock rock :max-rock rock}))))
        land
        (range width)))
    land-data
    (range height)))
